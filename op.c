...
{
    nes->regs.status = (*mem & NEGATIVE_FLAG ) | (*mem & OVERFLOW_FLAG);
...
{   
    nes->regs.pc--; //?
    nestor_st_push(nes, (uint8_t)(nes->regs.pc >> 8));
    nestor_st_push(nes, (uint8_t) nes->regs.pc);
    jmp(nes, val);
}
